...
GetCharacterPageLayoutInfo :: proc(state: ^st.state, cell_size: f32) -> CharacterPageLayoutInfo {
    ...
    return {grid_start_x, avatarCenterX}
}

GetCharacterGridLocations :: proc(char: ^inv.Character, top_margin: f32, cell_size: f32, start_x: f32) -> [dynamic]GridLocation {
    locs := make([dynamic]GridLocation, context.temp_allocator)
    current_y: f32 = top_margin + 50
...
GetCharacterSlotRects :: proc(state: ^st.state, centerX: f32) -> map[inv.EquipmentSlot]rl.Rectangle {
    centerY := (state.window.height + ui.TOP_MARGIN) / 2
    avatar_rect := rl.Rectangle{f32(i32(centerX - AVATAR_WIDTH / 2)), f32(i32(centerY - AVATAR_HEIGHT / 2)), AVATAR_WIDTH, AVATAR_HEIGHT}
